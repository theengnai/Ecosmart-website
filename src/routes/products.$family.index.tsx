      ...
      <section className="border-t border-line/60 bg-canvas px-5 py-24 md:px-10 md:py-32">
        <div className="mx-auto grid max-w-7xl gap-12 md:grid-cols-[1fr_1.2fr] md:gap-16">
          ...
          <div className="divide-y divide-line/50 rounded-2xl border border-line/60 bg-canvas-2/40">
            {PU_SPECS.map(([k, v]) => (
              <div key={k} className="grid grid-cols-[minmax(0,0.8fr)_minmax(0,1.2fr)] items-baseline gap-4 px-6 py-4 md:gap-6 md:px-7 md:py-5">
                <span className="min-w-0 text-left font-mono text-[0.62rem] uppercase tracking-[0.28em] text-ink-soft">{k}</span>
                <span className="min-w-0 justify-self-end text-right text-sm text-ink md:text-base">{v}</span>
              </div>
            ))}
          </div>
        </div>
